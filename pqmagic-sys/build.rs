...
fn build_from_source() -> std::path::PathBuf {
  ...
  cfg.define("ENABLE_TEST", "OFF").define("ENABLE_BENCH", "OFF");
  for alg_define in DIS_ALG_CMAKE_ARR {
    cfg.define(alg_define, "OFF");
  }
...
fn main() {
  #[cfg(feature = "adv")]
  compile_error!(
    "Open source version only support PQMagic-std. Please disable `adv` or contact the author for further high performance support."
  );
  #[cfg(all(
